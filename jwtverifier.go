...
// IVerifier defines the behavior necessary for verifying a token for the
// purposes of the middleware provided in this package
type IVerifier interface {
	VerifyToken(tokenString string) (token *Token, err error)
}
...
type Claims struct {
	...
	UserID      string       `json:"https://meta/userId,omitempty"`
	Email       string       `json:"https://meta/email,omitempty"`
...
func (v *Verifier) keyFunc(token *jwt.Token) (interface{}, error) {
	...
	// Verify the issuer claim. We remove any trailing slashes to ensure that we
	// don't have mismatches due to a missing slash. This brings it in line with
	// how some of the other languages' libraries work.
	issuer := strings.TrimRight(v.TenantURL, "/")
	issuerClaim := strings.TrimRight(claims.Issuer, "/")
	if issuerClaim == "" || issuerClaim != issuer {
		return nil, fmt.Errorf("bad token: issuer is '%s' when it should be '%s'", issuerClaim, issuer)
	}
...
// MarshalJSON is a custom marshaller for AudienceList that is needed because
// the Audience claim can be a string or an array. Fun stuff.
...
// UnmarshalJSON is a custom unmarshaller for AudienceList that is needed because
// the Audience claim can be a string or an array. Fun stuff.
